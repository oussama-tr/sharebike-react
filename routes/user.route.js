...
router.post('/mobile/addCard/', (req, res) => {
  passport.authenticate("mobile", { session: false }, function(err, user, info)
  {
    if (err) res.send(err);
    else if (user == false)
      res.status(401).send(info);
    else {
      stripeApi.customers.createSource(
          ''+user._id,
...
router.post('/mobile/removeCard/', (req, res) => {
  passport.authenticate("mobile", { session: false }, function(err, user, info)
  {
    if (err) res.send(err);
    else if (user == false)
...
router.get('/mobile/getCards/', (req, res) => {
  passport.authenticate("mobile", { session: false }, function(err, user, info)
  {
    if (err) res.send(err);
    else if (user == false)
      ...
      stripeApi.customers.listSources(
          ''+user._id,
...
router.get('/mobile/getCustomer/', (req, res) => {
  passport.authenticate("mobile", { session: false }, function(err, user, info)
  {
    if (err) res.send(err);
    else if (user == false)
